...
# ## Exercise 3
#
# Consider the "target" model $p$ consisting of $\text{Norm}(\mu,\sigma^2)$, where $\mu \sim \text{Unif}([-5,+5])$ and $\sigma^2 \sim \text{Unif}([1,5])$ are marginalized out.  Thus $p$ is a continuous average (better: integral) of these normal distributions, and its density is not obviously in closed form.
#
...
# 6. Now use the additional information that $0 < 1 \leq \sigma^2 \leq 5 < 10$ to produce a constant $M > 0$ that bounds the weights of $\~q$, and use it to run rejection sampling.  Plot its histogram in comparison to $p$.
